...
void runStateMachine() {
  ...
  switch (state) {
    ...
    case DRIVE_FORWARD:
      ...
      if (WallAccelPosition(86*31,30*31)) {
      ZeroEncoders();  
        state = PUSH_POT;
      }
      break;
    ...
    case MOVE_TO_IGNITER:
      ...
      if (WallAccelPosition(-20*31,-10*31)) {
        ZeroEncoders();
        StopDrivePower();
        state = PRESS_IGNITER;
      }
    ...
    case MOVE_UP_BURNER:
      ...
      if (WallAccelPosition(5*31,11*31)) {
        ZeroEncoders();
        StopDrivePower();
        state = DROP_BALL;
      }
  ...
  // Use the servo to drop the ball
    ...
    Serial.println("MRW");
      if (WallAccelPosition(82*31,-14*31)) {
        ZeroEncoders();
